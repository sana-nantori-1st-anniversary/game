var problemList = [level1, level2, level3];
...
var Game = function(param) {
  this.buttons = [
...

    {
      ...
      textAlign: "center",
      mouseOver: 0,
      bColor: (bColor = ignorePunctuation ? "red" : "white"),
      fColor: ignorePunctuation ? "white" : "black",
      bOverColor: ignorePunctuation ? "white" : "red",
      fOverColor: ignorePunctuation ? "black" : "white",
      labelOffset: 10,
      label: ignorePunctuation
        ? "句読点を無視する(変更するとやり直しになります)"
        : "句読点を無視しない(変更するとやり直しになります)",
      onClick: function() {
        ignorePunctuation = !ignorePunctuation;
        this.label = ignorePunctuation
          ? "句読点を無視する(変更するとやり直しになります)"
          : "句読点を無視しない(変更するとやり直しになります)";
...

    {
      ...
      textAlign: "center",
      mouseOver: 0,
      bColor: (bColor = ignoreSymbol ? "red" : "white"),
      fColor: ignoreSymbol ? "white" : "black",
      bOverColor: ignoreSymbol ? "white" : "red",
      fOverColor: ignoreSymbol ? "black" : "white",
      labelOffset: 10,
      label: ignoreSymbol
        ? "記号を無視する(変更するとやり直しになります)"
        : "記号を無視しない(変更するとやり直しになります)",
      onClick: function() {
        ignoreSymbol = !ignoreSymbol;
        this.label = ignoreSymbol
          ? "記号を無視する(変更するとやり直しになります)"
          : "記号を無視しない(変更するとやり直しになります)";
  ...
  this.draw = function() {
    ...
    if (this.isActive) {
      ...
      ctx.font =
        ctx.measureText(p.text).width > 800 ? "18px JKfont" : "24px JKfont";
      ctx.fillStyle = "black";
      ctx.fillText(p.text, 80, 160);

      ctx.font = "24px JKfont";
      ctx.font =
        ctx.measureText(p.getSelectText()).width > 800
          ? "18px JKfont"
          : "24px JKfont";
      ctx.fillStyle = "gray";
  ...
  this.update = function() {
    if (this.isActive) {
      ...
      this.nowTime =
        gameSec[param.level] -
        Math.floor((date.getTime() - this.startTime) / 1000);
  ...
  this.keyPress = function(e) {
    if (this.isActive) {
      if (
        this.ignopreList.find(function(element) {
        ...
        this.keyCount++;
        this.accuracy =
          ((this.keyCount - this.missCount) / this.keyCount) * 100;
        ...
        if (tmpList.length != 0) {
          ...
          if (
            p.tCount > 0 &&
            p.alphabet[p.tCount - 1][p.selectList[p.tCount - 1][0]] == "n" &&
            e.key == "n"
          ) {
            //一つ前のんの選択を'nn'にする
  ...
  this.click = function(e) {
    if (e.target.id == "canvas") {
      for (let i of this.buttons) {
        if (
          i.x <= e.offsetX &&
          e.offsetX <= i.x + i.width &&
          (i.y <= e.offsetY && e.offsetY <= i.y + i.height)
        ) {
          playSE(decideSound);
          i.onClick();
        }
