var Result = function(param) {
  ...
  this.speed = (
    (param.keyCount - param.missCount) /
    gameSec[param.level]
  ).toFixed(1);
  ...
  this.buttons = [
...

    {
      ...
      bColor: "DodgerBlue",
      fColor: "white",
      bOverColor: "DeepSkyBlue",
      fOverColor: "white",
      labelOffset: 10,
      label: "ツツイのツイ",
      onClick: () => {
        let url = "https://1st.natorisana.love/";
        let text = `レベル${param.level + 1}で${
          this.finalScore
        }点獲得！！ (正確率:${param.accuracy.toFixed(1)}％, 速さ:${
          this.speed
        }key/s)`;
        window.open(
          `https://twitter.com/share?url=${url}&text=${text}&hashtags=名取さなのつよつよタイピング`
        );
      }
    }
  ];
  ...
  this.draw = function() {
    ...
    ctx.fillText(
      "最終スコア : " + param.score + " × " + param.accuracy.toFixed(1) + "%",
      20,
      200
    );
